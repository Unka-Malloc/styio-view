...
import 'package:vityo_app/src/view_ide/agent_client/agent_prompt_profile_store.dart';
...
import 'package:vityo_app/src/view_render/agent_workbench/agent_surface.dart';
...
void main() {
  testWidgets('agent surface displays active workspace coding skills', (
    ...
    expect(find.text('Vityo Feature Loop'), findsOneWidget);
  ...
  testWidgets('agent surface displays applied patch operation counts', (
    ...
    expect(
      find.text('Applied Agent patch patch-replace atomically.'),
      findsOneWidget,
    );
  ...
  testWidgets('agent surface renders skipped no-op patch files', (
    ...
    expect(
      find.text('Agent patch patch-noop produced no text changes.'),
      findsOneWidget,
    );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        feedbackTurns.single.text,
        contains('Agent patch patch-failed targets an unavailable resource.'),
      );
      final patchApplication =
          adapter.requests.last.context.agent.lastPatchApplication!;
      ...
      expect(
        ...
        'Agent patch patch-failed targets an unavailable resource.',
      );
      expect(
        adapter
            .requests
  ...
  testWidgets('agent surface displays unsupported file-operation rejection', (
    tester,
  ) async {
    final editorController = EditorSessionController(
      initialDocument: const DocumentState(
    ...
    expect(
      find.text(
        'Agent patch patch-1 contains an unsupported or oversized edit.',
      ),
      findsOneWidget,
    );
    expect(find.textContaining('Changed files:'), findsNothing);
    expect(find.textContaining('Created files:'), findsNothing);
    expect(find.textContaining('Deleted files:'), findsNothing);
  });

  testWidgets('agent file-operation rejection reaches next provider request', (
    tester,
  ) async {
    final editorController = EditorSessionController(
      initialDocument: const DocumentState(
    ...
    expect(
      feedbackTurns.single.text,
      contains('contains an unsupported or oversized edit'),
    );
    final patchApplication =
        adapter.requests.last.context.agent.lastPatchApplication!;
    ...
    expect(patchApplication.applied, isFalse);
    expect(patchApplication.createdDocumentIds, isEmpty);
    expect(patchApplication.deletedDocumentIds, isEmpty);
    expect(
      adapter
          .requests
          .last
  ...
  testWidgets('agent surface follows controller patch application state', (
    ...
    await controller.sendPrompt();
    controller.updatePrompt('Follow up while patch applies.');
    final applyResult = controller.applyPendingPatch(patchRuntime.applier);
