    ...
    return const IdeCapabilityFrameworkSnapshot(
      ...
      entries: <IdeCapabilityDescriptor>[
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'Workspace text search service, in-memory search index snapshot, WorkspaceSearchIndexController stale-revision refresh execution, WorkspaceSearchIndexFileSystemWatcherBinding File System Manager watch refresh execution, persistent index invalidation key contract, symbol search service with semantic snapshot source/confidence propagation, file quick open service, replace preview contract with before/after diff summary, virtualized replace-preview document windows, persisted multi-file diff expansion state, replace apply confirmation, search history persistence, persisted result filter state, search history/index/filter/expansion summaries in the user surface, agent search command, and match-level navigation callback are wired.',
          todo:
        ...
        IdeCapabilityDescriptor(
          ...
          ownerPath: 'lib/src/view_ide/agent',
          summary:
              'Structured plan, diagnostics, code patch, IDE command, command result, streaming content deltas, AgentCodingSessionController streaming adapter dispatch, WorkspaceEdit bridge, Source Control Agent context bridge, failed-test rerun context, StyioService readiness checkpoints, SemanticSnapshotPanelViewModel Problems/Refactor context, checkpoint result prompt replay, patch application loop, persisted coding session history, serializable AgentCodingSessionCheckpoint recovery facts, AgentCodingSessionRecoveryPlan retry/failover/replay facts, AgentCodingSessionRecoveryCommandPlan command routing facts, AgentCodingSessionRecoveryRequestDraft prompt restoration, explicit AgentCodingSessionRecoveryDispatchResult confirmation gates, Agent Surface recovery action controls, Agent Surface recovery dispatch status feedback, history restore/persistence failure output events, RuntimeOutputLiveBuffer agent activity publishing, AgentProviderSelectionContext prompt injection, provider credential/execution readiness selection, provider readiness context serialization, provider selection status rendering, and embeddable activity history surface with controller snapshot binding, failed-record reason, and diagnostic summary count visibility.',
