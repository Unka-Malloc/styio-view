    ...
    return const IdeCapabilityFrameworkSnapshot(
      ...
      entries: <IdeCapabilityDescriptor>[
        ...
        IdeCapabilityDescriptor(
          ...
          todo:
              'TODO: bind PlatformSecureCredentialStorageAdapter to OS SecretStorage/Keychain/Credential Manager/libsecret production backends.',
          runtimeMaturityBlocking: false,
          references: <String>[
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'LanguageServiceStatusSurface and LanguageServiceStatusController map StyioService runtime events, capability health summaries, Styio language provider readiness derived from StyioService capability snapshots, syntax-validation readiness, semantic-fact readiness, and unavailable capability summaries into ValueListenable status snapshots for editor, shell, and Agent consumers.',
        ...
        IdeCapabilityDescriptor(
          ...
          ownerPath: 'lib/src/view_ide/workspace',
          summary:
              'Source Control surface is wired to dirty editor documents, Git porcelain status parsing, diff preview, diff review summaries, parsed diff hunks, SourceControlHunkSelectionState multi-hunk selection, SourceControlHunkDiscardConfirmationPlan destructive discard confirmation, hunk action selection plans, selected hunk patch generation, Git partial patch execution provider, shell hunk action execution routing, hunk action result rows, Agent context snapshots with hunk selection and partial patch results, virtualized diff window binding, diff confirmation plans and controls, Agent context snapshots with pending action plans and last action results, staging action contracts, Agent stage/unstage command routing, branch-switch plan command routing, commit-draft plan command routing, persisted commit drafts, commit dialog state validation, commit draft summaries, branch picker summaries, branch switch plans, Git branch switch provider, history summaries, expandable history rows, action planning and confirmation, Git stage/unstage/discard/commit action provider, Git branch/history provider contracts, non-Git provider adapter descriptors and surface summaries, file open, and save-all handoff.',
          todo:
              'TODO: bind hunk discard confirmation to concrete modal UI and restore diff selection from a persisted diff-session DataStore owner.',
          runtimeMaturityBlocking: false,
          references: <String>[
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'Testing surface is wired to runTests, native tool testResult records, TestRunProvider registry, TestDiscoveryProvider registry, TestingProviderCatalog controller fallback, TestingProviderCatalog health snapshots, TestingProviderRetryPlan retry action facts, test run configurations, persisted run configuration sets, run/debug selected configuration controls, test tree model, persisted test run history, persisted failed-test retry history, runtime task lifecycle snapshots, persisted runtime task history, output stream subscription plans, RuntimeOutputLiveBuffer test-result publishing, failed-test rerun planning, failed-test interaction, per-failed-test debug cancellation routing, FailedTestDebugCancellationRoute process-handle metadata, rerun-failed handoff, failed-test DebugLaunchRoutePlan bridge, debug launch route plans, failure navigation actions, DebugRuntimeExecutionAdapter result visibility, cancellation telemetry, debug retry handoff, and CTest result parsing.',
          todo:
              'TODO: bind failed-test debug cancellation routes to concrete debug adapter and test runner process termination.',
          runtimeMaturityBlocking: false,
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'WorkspaceDiagnosticsSnapshot and WorkspaceDiagnosticsProviderRegistry provide shared workspace problem facts for Problems, Agent, and code actions, including document grouping, source grouping, persisted filters, workspace diagnostic stream snapshots, WorkspaceDiagnosticsRuntimeOutputBinding output-panel events, WorkspaceDiagnosticsProducerExecutionPlan toolchain execution triggers, WorkspaceDiagnosticsProducerLifecycleController progress/cancel snapshots and registered plan lookup, diagnostics producer cancellation adapter contracts, WorkspaceDiagnosticsController producer cancellation dispatch, runtime task cancellation metadata, attached quick-fix facts, source-kind classification, workspace quick-fix confirmation plans, WorkspaceQuickFixTelemetryStore persisted review outcomes, preview/apply action routing, and native tool result diagnostic snapshots.',
          todo:
              'TODO: bind diagnostics producer process-handle adapters to real ToolchainManager/native process handles for every production provider.',
          dependencies: <String>[
            'foundation.registry',
            'service.styio-language',
          ],
