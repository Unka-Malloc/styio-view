# Styio Coding Agent — Architecture
...
The package depends on `packages/styio_agent_protocol` and pure runtime libraries only. It cannot
depend on `products/styio_ide`, Flutter, IDE document types, widget state, or IDE data stores.
...
flowchart TB
    ...
    Tools --> Host
    Delegate --> Host
    Host --> Client["Styio IDE or headless host"]
```
...
- protocol integration proves the real Styio IDE seam.
