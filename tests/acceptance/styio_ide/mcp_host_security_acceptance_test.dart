...
import 'package:styio_ide/src/ide/agent_client/mcp/ide_mcp_server.dart';
import 'package:styio_ide/src/ide/agent_client/mcp/workspace_root_registry.dart';
import 'package:styio_ide/src/ide/agent_client/tools/context_export_service.dart';
import 'package:styio_ide/src/ide/agent_client/tools/ide_tool_catalog.dart';
import 'package:styio_ide/src/ide/agent_client/tools/tool_security_policy.dart';
import 'package:styio_ide/src/ide/extensions/mcp_extension_registry.dart';
...
Future<void> _dynamicDiscoveryAndBoundedRevisionedContext() async {
  final temporary = await Directory.systemTemp.createTemp('styio-mcp-host-');
  ...
  try {
    ...
    _expect(
      contextResult['workspaceRevision'] == 12 &&
          contextResult['provenance'] == 'styio-ide-facts' &&
    ...
    _expect(
      decodedResource['workspaceRevision'] == 12 &&
          decodedResource['provenance'] == 'styio-ide-facts',
...
Future<void> _rootsCapabilitiesAndExtensionsRevokeImmediately() async {
  final temporary = await Directory.systemTemp.createTemp('styio-mcp-live-');
  final roots = WorkspaceRootRegistry();
  await roots.replaceRoots(
