...
| S2 | Profile | User profile, local-first, optional sync | VityoProfileSnapshot: local-first and sync-optional; Agent runtime profile is external to the IDE | Local profile storage anchors exist | Profile snapshot model; optional ProfileSync; retire IDE-owned provider profile | `view_ide/environment/configuration/` | Profile local-first test; missing-sync test | No login-required profile; no IDE-owned model credentials |
...
| # | Capability Family | Benchmark Standard | Vityo Target | Current | Gap | Repo Anchors | Validation | Non-goals |
|---|---|---|---|---|---|---|---|---|
| A1 | Agent Context | Workspace-scoped context: active document, selection, diagnostics, project graph, runtime events | IDE exports a scoped, redacted, revision-bound context projection through the Agent protocol | Context and redaction anchors exist | Align all exports with shared protocol and remove provider-shaped IDE context | `view_ide/agent_client/` | Context serialization, redaction, revision, and scope-exclusion tests | No raw secrets/tokens or implicit full-workspace export |
| A2 | Agent Permissions | Permission requests and explicit user approval | Agent runtime requests scoped authority; Vityo presents the request and records the decision | Permission request/decision surfaces exist | Complete protocol receipt and audit projection | `view_ide/agent_client/`, `view_render/agent_workbench/` | Permission request, denial, and receipt tests | No silent permission bypass |
| A3 | Change Workflow | Preview diff, apply, rollback; structured edits | Compatible Agent proposes revision-bound changes; Vityo previews and applies them through workspace transactions | FileChangePreview and transaction anchors exist | End-to-end protocol proposal/receipt closure | `view_ide/agent_client/`, `ide/workspace/` | Preview no-mutation, stale-revision rejection, apply, and rollback tests | No Agent direct file write |
| A4 | Agent Client | Open client protocol, independent companion runtime, disconnected operation | Connect Vityo Coding Agent or another compatible Agent through the versioned protocol; the IDE never connects to a model provider | Shared protocol and IDE Client anchors exist; legacy IDE provider/controller code also remains | Atomically retire IDE model/provider and tool-loop ownership | `packages/vityo_agent_protocol/`, `view_ide/agent_client/` | Protocol interoperability; disconnected IDE-loop test; product-line boundary gate | No model/provider SDK or credential routing in IDE |
| A5 | Agent Workbench | Plan, task state, permissions, changes, and verification | First-class Agent Workbench projects protocol state and keeps user control visible | Agent presentation and command anchors exist | Complete plan and verification-receipt views | `view_render/agent_workbench/`, `view_ide/commands/` | Keyboard navigation; permission/change/receipt projection tests | No chat-only product definition |
