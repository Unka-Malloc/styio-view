#!/usr/bin/env python3
"""Focused quality entry point for Styio product lifecycles."""
...
from styio_validation_receipt import (
...
_FINGERPRINT_ROOTS = (
    ...
    "products/styio_ide/test",
    "products/styio_ide/integration_test",
    "products/styio_ide/benchmark",
...
_AGENT_FINGERPRINT_ROOTS = (
    ...
    "packages/styio_agent_protocol",
    "scripts",
    "docs/plan/styio-coding-agent",
)
...
def run(
    ...
    print(
        f"[styio-quality] {cwd.relative_to(ROOT) or '.'}: {' '.join(command)}",
        flush=True,
    )
...
def cutover() -> int:
    ...
    commands = (
        ...
        ([dart, "analyze"], ROOT / "packages" / "styio_agent_protocol"),
        ([dart, "test"], ROOT / "packages" / "styio_agent_protocol"),
        ([dart, "analyze"], ROOT / "products" / "styio_coding_agent"),
        ([dart, "test"], ROOT / "products" / "styio_coding_agent"),
        ([flutter, "analyze"], ROOT / "products" / "styio_ide"),
        ([flutter, "test", "test/styio_ide_smoke_test.dart"],
         ROOT / "products" / "styio_ide"),
    )
...
def workspace_transactions() -> int:
    dart = tool("dart")
    product = ROOT / "products" / "styio_ide"
    commands = (
        ([sys.executable, "scripts/check_product_line_boundaries.py"], ROOT),
...
def agent_client_protocol() -> int:
    ...
    commands = (
        ...
        (
            ...
            {
                **os.environ,
                "STYIO_ACCEPTANCE_PRIVATE": "present",
            },
        ),
    )
...
def ide_quality() -> int:
    ...
    commands = (
        ...
        (
            [
                sys.executable,
                "packaging/styio_ide/desktop_delivery.py",
                "--repo-root",
                ".",
            ],
            ROOT,
...
def main() -> int:
    ...
    if (args.product, args.suite) == ("ide", "full"):
        receipt = args.receipt or (
            ROOT / "artifacts" / "validation" / "styio-ide-full.json"
        )
        return ide_full(
