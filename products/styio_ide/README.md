# Styio IDE Flutter Shell

这是 `Styio IDE` 的 Flutter 主实现入口，负责承载：

当前 Flutter package 与主实现目录已统一为 `products/styio_ide` / `styio_ide`，平台 runner 与 bundle id 也使用 Styio IDE 命名。
...
1. 检查 `products/styio_ide/pubspec.yaml` 的 Styio IDE editor 元数据。
...
1. IDE 实现只落在 `products/styio_ide`，Coding Agent 实现只落在 `products/styio_coding_agent`
...
4. 跨产品 DTO、能力协商与会话信封只落在 `packages/styio_agent_protocol`
...
21. 工作区侧栏新增 `Required Handoffs` 卡，只表达 `Styio IDE` 还需要 `styio` / `pafio` 提供哪些 machine contract，不替上游做内部实现规划
