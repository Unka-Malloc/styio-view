...
import 'package:vityo_app/src/view_ide/backend_toolchain/execution_route_summary.dart';
...
void main() {
  ...
  group('3) Command availability and runtime route messaging matrix', () {
    test('VityoCommandRegistry registers surface commands for mobile', () {
      final surfaceCommands = VityoCommandRegistry.surfaceCommands.toList(
        growable: false,
      );
    ...
    test('primary commands include navigation and search ops for mobile', () {
      final primaryIds = VityoCommandRegistry.primaryCommands
          .map((c) => c.id)
          .toSet();
    ...
    test('mobile-friendly workflow commands exclude debug-only commands', () {
      final workflowIds = VityoCommandRegistry.workflowCommands
          .map((c) => c.id)
          .toSet();
    ...
    test('deployment commands are accessible for mobile publish flows', () {
      final deployIds = VityoCommandRegistry.deploymentCommands
          .map((c) => c.id)
