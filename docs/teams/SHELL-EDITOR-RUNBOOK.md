...
13. 当上游 `styio` 语法仍在实现中时，`frontend/styio_view_app/lib/src/language/styio_syntax_highlighter.dart` 只能提前提供宽容 token、literal-safe token、typed-declaration-safe semantic、parameter semantic 和 block-range 支持；language service 可以复用这些结果提供 completion 和 hover，但不得把 mock 支持描述成编译器已可执行能力，compile/run 仍必须通过 adapter capability gap 或真实 handoff 表达。
