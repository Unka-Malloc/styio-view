...
products/
  ...
  styio_coding_agent/           # separately owned product; never imported
...
`products/styio_ide/lib/src/app` may depend on IDE domain, presentation, and the protocol package.
...
After the cutover, ordinary IDE nodes own only `products/styio_ide`, IDE-specific gates/docs, and
...
The neutral protocol package implements ACP-compatible core session semantics:
...
| Agent Client protocol | `ide/agent_client`, neutral protocol schema/DTOs, process supervisor |
