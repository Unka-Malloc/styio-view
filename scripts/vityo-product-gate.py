...
Usage:
    python3 scripts/vityo-product-gate.py
...
def check_view_render_imports_view_ide() -> bool:
    ...
    view_render_dir = (
        REPO_ROOT / "products" / "vityo_app" / "lib" / "src" / "view_render"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Vityo Product Gate")
    parser.add_argument(
        "--mode",
    ...
    if not check_test_anchor(
        "products/vityo_app/test/*command*",
        "Command registry tests",
    ):
        failures += 1
    if not check_test_anchor(
        "products/vityo_app/test/*agent*",
        "Agent context/permission/patch tests",
    ):
        failures += 1
    if not check_test_anchor(
        "products/vityo_app/test/*diagnostic*",
    ...
    if failures > 0:
        ...
        print("All Vityo product gate checks passed.")
