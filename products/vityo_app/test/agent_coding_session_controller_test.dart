...
import 'package:vityo_app/src/view_ide/agent_client/agent_coding_session_history_store.dart';
...
import 'package:vityo_app/src/view_ide/agent_client/agent_registry.dart';
import 'package:vityo_app/src/view_ide/agent_client/agent_provider_route_executor.dart';
import 'package:vityo_app/src/view_ide/agent_client/agent_tool_call_dispatcher.dart';
import 'package:vityo_app/src/view_ide/agent_client/agent_tool_call_execution_plan.dart';
import 'package:vityo_app/src/view_ide/agent_client/agent_tool_registry.dart';
import 'package:vityo_app/src/view_ide/agent_client/agent_tool_permission_policy_store.dart';
import 'package:vityo_app/src/view_ide/agent_client/agent_workspace_snapshot.dart';
import 'package:vityo_app/src/view_ide/agent_client/agent_workspace_snapshot_store.dart';
...
import 'package:vityo_app/src/view_ide/agent_client/agent_tool_session_transcript.dart';
...
void main() {
  ...
  test(
    ...
    () async {
      ...
      await controller.sendPrompt();
      final patchRuntime = _AgentPatchTestRuntime.fromEditor(editorController);
      final applied = await controller.applyPendingPatch(patchRuntime.applier);
  ...
  test('agent coding session commits pending code patch transaction', () async {
    final editorController = EditorSessionController(
      initialDocument: const DocumentState(
        documentId: 'main.styio',
        text: 'value = 1\n',
    ...
    await controller.sendPrompt();
    final patchRuntime = _AgentPatchTestRuntime.fromEditor(editorController);
    final result = await controller.applyPendingPatch(patchRuntime.applier);

    expect(result?.applied, isTrue);
    expect(controller.pendingPatch, isNull);
    expect(controller.lastPatchApplicationResult?.appliedEditCount, 1);
    expect(
      patchRuntime.revisions.snapshot().document('main.styio').text,
      'value = 2\n',
    );
  ...
  test(
    ...
    () async {
      ...
      await controller.sendPrompt();
      final snapshotService = AgentWorkspaceSnapshotService(
        editorController: editorController,
      );
      await controller.capturePendingPatchSnapshot(snapshotService);
      final patchRuntime = _AgentPatchTestRuntime.fromEditor(editorController);
      final result = await controller.applyPendingPatch(patchRuntime.applier);
      final appliedDocument = patchRuntime.revisions.snapshot().document(
        'main.styio',
      );
      editorController.loadDocument(
        DocumentState(
          documentId: appliedDocument.resourceId,
          text: appliedDocument.text,
          revision: appliedDocument.revision,
        ),
      );
      await controller.buildWorkspaceRevertPlan(snapshotService);
      ...
      final revertResult = await controller.applyLastWorkspaceRevertPlan(
        patchRuntime.applier,
      );
  ...
  test('agent coding session restores persisted workspace snapshot', () async {
    ...
    final patchRuntime = _AgentPatchTestRuntime.fromEditor(editorController);
    await patchRuntime.applier.apply(patch);
    final appliedDocument = patchRuntime.revisions.snapshot().document(
      'main.styio',
    );
    editorController.loadDocument(
  ...
  test(
    ...
    () async {
      ...
      await controller.sendPrompt();
      final patchRuntime = _AgentPatchTestRuntime.fromEditor(editorController);
      await controller.applyPendingPatch(patchRuntime.applier);
  ...
  test(
    'agent coding session applies pending multi-resource patch atomically',
    () async {
      final editorController = EditorSessionController(
        initialDocument: const DocumentState(
          documentId: 'main.styio',
      ...
      await controller.sendPrompt();
      final patchRuntime = _AgentPatchTestRuntime.fromEditor(
        editorController,
        additionalDocuments: const <String, String>{
          'other.styio': 'name = old\n',
        },
      );
      final result = await controller.applyPendingPatch(patchRuntime.applier);
      final snapshot = patchRuntime.revisions.snapshot();
  ...
  test(
    ...
    () async {
      ...
      controller.updatePrompt('Change workspace.');
      await controller.sendPrompt();
      final firstResult = controller.applyPendingPatch(patchRuntime.applier);
      await patchRuntime.previewStarted;
      controller.updatePrompt('Do not send while applying.');
      expect(controller.canSend, isFalse);
      expect(await controller.sendPrompt(), isNull);

      final secondResult = await controller.applyPendingPatch(
        patchRuntime.applier,
      );
      patchRuntime.releasePreview();
  ...
  test(
    ...
    () async {
      ...
      controller.updatePrompt('Change workspace.');
      await controller.sendPrompt();
      final result = controller.applyPendingPatch(patchRuntime.applier);
      await patchRuntime.previewStarted;
...
final class _AgentPatchTestRuntime {
  _AgentPatchTestRuntime(
    Map<String, String> documents, {
    bool blockPreview = false,
  }) : revisions = InMemoryWorkspaceRevisionService(
         initialDocuments: documents,
       ) {
    final baseTransactions = RevisionedWorkspaceTransactionService(revisions);
    if (blockPreview) {
      final blocking = _BlockingWorkspaceTransactionService(baseTransactions);
      _blockingTransactions = blocking;
  ...
  factory _AgentPatchTestRuntime.fromEditor(
    EditorSessionController editorController, {
    Map<String, String> additionalDocuments = const <String, String>{},
    bool blockPreview = false,
  }) {
    return _AgentPatchTestRuntime(<String, String>{
  ...
  late final WorkspaceTransactionService transactions;
  late final AgentCodePatchApplier applier;
  _BlockingWorkspaceTransactionService? _blockingTransactions;

  Future<void> get previewStarted {
    final blocking = _blockingTransactions;
    if (blocking == null) {
      throw StateError('Preview blocking is not enabled.');
    }
    return blocking.previewStarted.future;
  }
...
final class _BlockingWorkspaceTransactionService
    implements WorkspaceTransactionService {
  _BlockingWorkspaceTransactionService(this._delegate);

  final WorkspaceTransactionService _delegate;
  final Completer<void> previewStarted = Completer<void>();
  final Completer<void> _releasePreview = Completer<void>();

  @override
  Future<WorkspaceTransactionPreview> preview(
    WorkspaceChangeSet changeSet,
  ) async {
    if (!previewStarted.isCompleted) {
      previewStarted.complete();
    }
    await _releasePreview.future;
    return _delegate.preview(changeSet);
  }

  @override
  Future<WorkspaceTransactionReceipt> commit(String previewId) {
    return _delegate.commit(previewId);
  }

  @override
  Future<WorkspaceTransactionReceipt> reject(String previewId) {
    return _delegate.reject(previewId);
  }

  @override
  Future<WorkspaceTransactionReceipt> rollback(String transactionId) {
    return _delegate.rollback(transactionId);
  }

  void release() {
    if (!_releasePreview.isCompleted) {
      _releasePreview.complete();
    }
  }
}
