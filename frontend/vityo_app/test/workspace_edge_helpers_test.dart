...
void main() {
  test('workspace quick open covers helper branches and empty projects', () {
    ...
    final copied = const WorkspaceQuickOpenQuery(
      pattern: 'old',
      maxResults: 1,
    ).copyWith(pattern: 'main', maxResults: 3);
  ...
  test(
    ...
    () async {
    final copied = const WorkspaceDocumentLinksQuery(
...
''',
    ...
    expect(result.links.single.kind, WorkspaceDocumentLinkKind.workspaceImport);
    expect(result.links.single.resolvedFilePath, 'src/lib/runtime.styio');
    },
  );

  test('workspace code actions expose helper getters and empty states', () async {
  ...
  test(
    ...
    () async {
    final searchQuery = const WorkspaceTextSearchQuery(
      pattern: 'ready',
    ).copyWith(
      pattern: 'emit',
      literal: false,
