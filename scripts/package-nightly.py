...
def package_linux(config: dict[str, object], output: Path, version: str) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="vityo-deb-") as raw_stage:
        ...
        shutil.copy2(ROOT / "packaging/linux/io.vityo.desktop", applications / "io.vityo.desktop")
        metainfo = stage / "usr/share/metainfo"
        metainfo.mkdir(parents=True)
        shutil.copy2(ROOT / "packaging/linux/io.vityo.metainfo.xml", metainfo / "io.vityo.metainfo.xml")
        icons = stage / "usr/share/icons/hicolor/512x512/apps"
        icons.mkdir(parents=True)
        shutil.copy2(require_file(ROOT / str(config["icon_relative_path"])), icons / "io.vityo.png")
