...
13. 上述 product gate (points 7-10) 均需要设置 `VITYO_PRODUCT_GATE=1` 和对应的外部 fixture（Styio/Pafio 可执行文件、hosted control plane endpoint）才会执行；默认 CI 不会运行这些 product workflow gate，仅验证本地可运行的单元测试和 widget 测试。\n\n> **Default CI vs Product Gate (English):** Default CI runs all workflows (audit.yml, repo-hygiene.yml, styio-audit.yml, project-coverage-gate.yml, local-ci-gate.yml, windows-native.yml) on every PR/push. These execute the delivery floor, Flutter release builds, coverage gates, and all policy gates. The opt-in product gates (ecosystem-product-gate.py, ecosystem sample workflow gate) require VITYO_PRODUCT_GATE=1 + external fixtures and are **not** part of default CI green. Default CI green does not prove product-matrix coverage, packaging/signing, distribution, or install/update/uninstall behavior. The terminal governance checkpoint 6fd0bfe7-3d65-429f-8a6d-fd0a0fc08092 requires launch-ready production release evidence to close.
14. 已知文档偏差（2026-06-25 审计确认）：`Vityo-Implementation-Gaps.md` 中 Remote/browser/virtual providers 与 Cache Contract 此前标记为 Closed，但实际上 `HostedWorkspaceFileSystemProvider` 未实现、`CacheStore<K,V>` 通用接口未发布、缓存 Level 2 持久化未实现——已降级为 Partially implemented。
