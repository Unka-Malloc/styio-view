    ...
    return const IdeCapabilityFrameworkSnapshot(
      ...
      entries: <IdeCapabilityDescriptor>[
        ...
        IdeCapabilityDescriptor(
          ...
          ownerPath: 'lib/src/view_ide/language/service',
          summary:
              'SemanticSnapshotProvider converts StyioService analysis into resolved elements/references, exposes StyioService-backed code action raw edit facts, code action apply/result telemetry, document rename safety facts, workspace rename safety facts, SemanticSnapshotEventBridge runtime-output events, Problems/Refactor panel event sink dispatching, SemanticSnapshotPanelEventStateController, SemanticSnapshotPanelEventStore persisted telemetry, publishes a feature coverage matrix for hover/definition/references/completion/rename/code action consumers, and only falls back to local snapshots when service semantic facts are missing.',
          todo:
              'TODO: bind semantic panel state stores to concrete Problems/Refactor UI view models and production telemetry retention.',
          references: <String>[
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'Agent provider route selection, credential readiness, endpoint probing, fallback selection, retry policy execution, remote service health reports, Foundation DataStore-backed health history, OpenAI-compatible/Responses provider routing, hosted backend connector parity action plans, HostedBackendRetryActionExecutor, and HostedControlPlaneRetryTransport are wired.',
          todo:
              'TODO: connect hosted retry actions to settings recovery handlers, published reopen/export endpoints, and production telemetry sinks.',
          references: <String>[
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'Workspace text search service, in-memory search index snapshot, WorkspaceSearchIndexController stale-revision refresh execution, persistent index invalidation key contract, symbol search service, file quick open service, replace preview contract with before/after diff summary, replace apply confirmation, search history persistence, persisted result filter state, search history/index/filter summaries in the user surface, agent search command, and match-level navigation callback are wired.',
        ...
        IdeCapabilityDescriptor(
          ...
          ownerPath: 'lib/src/view_ide/commands',
          summary:
              'Command Palette surface is wired to StyioCommandRegistry, category contribution manifests, shell command execution, reusable query scoring, overlay selection state, live display preference updates, top-level SettingsSurface preference saves, shell-level DataStore preference hydration/persistence, app bootstrap preference hydration, category filter chips, display preferences, Settings UI preference controls, Up/Down/Enter keyboard navigation, typed input draft contracts, keyboard shortcuts, keybinding remap persistence, conflict review contracts, persisted recent command ranking, recent command record hooks, and blocked command availability reasons.',
          todo:
              'TODO: add user-editable keybinding remap UI and conflict resolution actions.',
          references: <String>[
