...
///
/// Precondition: the neutral protocol package and canonical schema are present.
...
/// Styio extensions require a negotiated `styio/` namespace, and no legacy
...
Future<void> _canonicalAcpV1ContractReplacesPrivateEnvelope() async {
  ...
  _expectThrowsProtocol(
    () => validateStyioExtensionMethod('other/unsafe', const <String>{}),
    'invalid_extension_namespace',
  );
  _expectThrowsProtocol(
    () => validateStyioExtensionMethod('styio/test/write', const <String>{
      'styio/test/status',
    }),
    'capability_revoked',
  );
  ...
  final schema = File.fromUri(
    Platform.script.resolve(
      '../../../packages/styio_agent_protocol/schema/acp-v1.schema.json',
    ),
  );
  final schemaJson =
      jsonDecode(await schema.readAsString()) as Map<String, Object?>;
  _expect(
    schemaJson[r'$id'] == 'https://styio.dev/schema/agent-client-protocol/v1',
    'canonical schema must identify ACP v1',
  );

  final protocolSource = await File.fromUri(
    Platform.script.resolve(
      '../../../packages/styio_agent_protocol/lib/src/protocol.dart',
    ),
  ).readAsString();
...
Future<void> _negotiationConcurrentStreamingPermissionAndCancellation() async {
  _expect(
    Platform.environment.containsKey('STYIO_ACCEPTANCE_PRIVATE'),
  ...
  try {
    ...
    _expect(
      connection.metadata['styio.test/privateEnvironmentVisible'] == false,
...
AgentClientRegistry _registry(Map<String, String> modes) {
  final fixture = File.fromUri(
    Platform.script.resolve(
      '../fixtures/styio_ide/agent_client/fake_agent.dart',
    ),
  );
