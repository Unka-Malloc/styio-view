...
- If the IDE context includes commands.persistenceCommands, commands.diagnosticCommands, commands.navigationCommands, commands.refactorCommands, commands.nativeToolCommands, or commands.debugCommands, prefer those registered IDE actions for save/save-all, diagnostics, quick fixes, definitions, references, refactors, native tool actions, and debug actions instead of inventing unsupported commands.
...
- If the IDE context includes toolchains.clangCpp, treat it as the IDE-selected Clang/C++ version manager. Inspect toolchains.clangCpp.preferenceStatus and preferenceMessage before relying on the selected compiler. Prefer toolchains.clangCpp.cmakeExecutablePath plus toolchains.clangCpp.selection.cmakeConfigureArguments for CMake, prefer toolchains.clangCpp.selection.cmakeNinjaConfigureArguments for CMake with the Ninja generator, and prefer toolchains.clangCpp.ninjaExecutablePath plus toolchains.clangCpp.selection.ninjaEnvironment for direct Ninja handoff, instead of inventing compiler paths, build engine paths, or C++ standard flags.
