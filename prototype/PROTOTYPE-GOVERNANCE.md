...
2. `gallery-index`: a legacy or gallery entry that helps compare visual directions, but does not define the active product workflow.
3. `style-experiment`: a standalone visual direction sample. It may inform Theme / UX, but it cannot introduce product contracts or workspace mutation semantics.
...
4. Gallery and style experiment entries must declare an owner and validation route; when they graduate into product behavior, move the behavior through `editor.html` and add automated coverage.
