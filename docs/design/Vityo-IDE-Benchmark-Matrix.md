...
|---|---|---|---|---|---|---|---|---|
| E1 | Document Model | Undo/redo, selection, cursor, multi-cursor, line-based editing, zero-latency input | Styio Source Buffer with undo/redo, selection, cursor, keyboard input; visual substitution is display-only | Document state, selection state, undo/redo, keyboard editing, glyph substitution cursor mapping exist | Multi-cursor, large-file degradation policy, full IME composition | `ide/editor/document/`, `ide/editor/selection/`, `ide/editor/controller/` | Widget tests for undo/redo, selection; large-file (>10K lines) perf test | No silent Source Buffer mutation; no VS Code TextModel clone |
| E2 | Visual Substitution | N/A (Styio-unique) | Glyph substitution for `->`, `|>`, `{ }` block surfaces, semantic decorations; user-toggleable | Glyph substitution engine exists, user toggle wired | Full semantic block surface driven by language service | `ide/editor/render_plan/`, `prototype/editor-modules/glyph-presets.js` | Toggle test: substitution on/off preserves Source Buffer; copy/paste uses raw text | No substitution during save; no substitution-bypassing diagnostic positions |
| E3 | Viewport / Layout | Responsive editor layout, minimap, breadcrumbs, split editors | Desktop/mobile dual layout, narrow viewport adaptation, workspace breadcrumbs | Desktop/mobile layouts exist; workspace breadcrumbs exist | Split editor, minimap | `view_render/shell/`, `ide/workspace/workspace_breadcrumbs.dart` | Narrow-viewport widget test; breadcrumb navigation test | No hardcoded desktop layout on mobile |
...
| L5 | Go-to-Definition | Symbol resolution to declaration, peek definition | DefinitionTarget from reference resolution | Definition target type exists; workspace definition navigator exists | Upstream reference resolution | `ide/workspace/workspace_definition.dart` | Definition navigation test | No string-match fallback as primary |
| L6 | Find References | Project-wide reference search, declaration/usage classification | ReferenceSpan with declaration/usage marking; workspace reference search | Reference spans in language contract; workspace reference search exists | Upstream reference provider | `ide/workspace/workspace_reference_search.dart` | Reference search test | No grep-based fallback as primary |
| L7 | Rename | Safe rename with preview, workspace edit application | RenamePlan with TextEdit collection; rename preview and apply | Rename plan type exists; workspace rename dialog exists | Upstream rename safety analysis | `ide/workspace/workspace_rename.dart` | Rename preview test; rename apply via transaction test | No global string replace bypassing document model |
| L8 | Code Actions | Quick fixes, source actions, lightbulb, preview/apply | DiagnosticQuickFix with preview/apply through workspace edit; capability gap when upstream blocked | Workspace code actions model exists | Upstream code action provider; capability gap rendering | `ide/workspace/workspace_code_actions.dart` | Quick fix preview test; apply via transaction test; capability gap test | No inline Source Buffer mutation |
...
| # | Capability Family | Benchmark Standard | Vityo Target | Current | Gap | Repo Anchors | Validation | Non-goals |
|---|---|---|---|---|---|---|---|---|
| P1 | Project Metadata | Package, workspace, dependency, and target view | Pafio metadata as the local project model; Styio machine-info as compiler truth; Platform hosted API as remote truth | Owner adapters and blocked-state projection exist | Keep UI aggregation derived from the three owner contracts | `view_ide/backend_toolchain/project_graph_contract.dart`, `owner_adapters/` | metadata adapter and source split tests | Never read Pafio private storage |
| P2 | Workspace Explorer | File tree, project view, dependency view, outline | File tree as one view; project graph as product model; workspace members, dependencies, targets, toolchain state visible | Workspace sidebar with project workflow, compiler handshake, required handoffs cards exist | Graph-first navigation (currently file-tree-first) | `ide/workspace/`, `view_render/shell/` | Explorer keyboard navigation test; graph summary test | No private directory structure inference |
| P3 | Dependency Management | Package manager integration, dependency tree, version resolution | sync/vendor workflow lanes through DependencySourceAdapter | Dependency source adapter and sync/vendor commands exist | Upstream Pafio dependency resolution | `view_ide/backend_toolchain/dependency_source_adapter.dart` | Sync/vendor lane state test | No Vityo-owned dependency resolver |
| P4 | Compiler Discovery | Compiler identity and capability discovery | Consume the system Styio machine contract directly | Styio machine-info adapter and generic non-Styio tool catalog exist | Surface clearer missing-system-compiler recovery | `owner_adapters/styio_compiler_adapter.dart`, `view_ide/toolchain/` | Owner adapter and toolchain status tests | No Styio install, update, pin, switch, or cache ownership |
...
| S2 | Profile | User profile, local-first, optional sync | A future provider-neutral profile remains local-first; Agent runtime profiles stay external to the IDE | Domain-specific shell, keybinding, test, and debug profiles exist; no general user/prompt profile store exists | Provider-neutral profile model and optional ProfileSync runtime | `view_ide/environment/configuration/`, `view_ide/commands/command_keybinding_profile.dart` | Domain-profile roundtrip tests; future missing-sync test | No login-required profile; no IDE-owned model credentials |
| S3 | Theme | Theme presets, token-level customization, semantic color mapping | Theme covering IDE shell, editor, semantic blocks, glyph, diagnostics, runtime, Agent Workbench, focus/selection | Theme tokens, presets, glyph presets exist | Theme editor UI; Agent Workbench theme; runtime surface theme | `view_render/theme/`, `prototype/editor-modules/theme-presets.js` | Theme preset roundtrip test; visual substitution toggle test | No commercial-brand default theme names |
...
| # | Capability Family | Benchmark Standard | Vityo Target | Current | Gap | Repo Anchors | Validation | Non-goals |
|---|---|---|---|---|---|---|---|---|
| A1 | Agent Context | Workspace-scoped context: active document, selection, diagnostics, project graph, runtime events | IDE exports a scoped, redacted, revision-bound context projection through the Agent protocol | Bounded context export, MCP root registry, sanitizer, and revision gates exist | Expand concrete context fact coverage | `ide/agent_client/tools/`, `ide/agent_client/mcp/` | Context serialization, redaction, revision, and scope-exclusion tests | No raw secrets/tokens or implicit full-workspace export |
| A2 | Agent Permissions | Permission requests and explicit user approval | Agent runtime requests scoped authority; Vityo presents the request and records the decision | Correlated one-shot permission requests, failure cleanup, and Workbench projection exist | Complete richer receipt/audit presentation | `ide/agent_client/`, `ide/workbench/agent_collaboration/`, `presentation/agent_workbench/` | Permission request, denial, failure cleanup, and receipt tests | No silent permission bypass |
| A3 | Change Workflow | Preview diff, apply, rollback; structured edits | Compatible Agent proposes revision-bound changes; Vityo previews and applies them through workspace transactions | Typed negotiated proposals route to preview and explicit commit without preview mutation | Expand first-party proposal/verification receipts | `ide/agent_client/`, `ide/workbench/agent_collaboration/`, `ide/workspace/` | Preview no-mutation, stale-revision rejection, apply, and rollback tests | No Agent direct file write |
| A4 | Agent Client | Open client protocol, independent companion runtime, disconnected operation | Connect Vityo Coding Agent or another compatible Agent through the versioned protocol; the IDE never connects to a model provider | Protocol-only boundary, concurrent supervision, bounded reducers, reconnect, and failure isolation exist | Add platform-appropriate alternate transports | `packages/vityo_agent_protocol/`, `ide/agent_client/` | Protocol interoperability; disconnected IDE-loop test; product-line boundary gate | No model/provider SDK or credential routing in IDE |
| A5 | Agent Workbench | Plan, task state, permissions, changes, and verification | First-class Agent Workbench projects protocol state and keeps user control visible | Immutable task/permission/change projection and presentation anchors exist | Complete plan and verification-receipt views | `ide/workbench/agent_collaboration/`, `presentation/agent_workbench/`, `view_ide/commands/` | Keyboard navigation; permission/change/receipt projection tests | No chat-only product definition |
...
| # | Capability Family | Benchmark Standard | Vityo Target | Current | Gap | Repo Anchors | Validation | Non-goals |
|---|---|---|---|---|---|---|---|---|
| Q1 | Input Latency | <16ms editor input latency, no perceptible typing lag | Editor input latency target defined; render slice/repaint scope bounded | Editor surface exists | Latency measurement baseline; large-file degradation policy | `ide/editor/`, `view_render/editor/` | Input latency widget test | No telemetry-based measurement |
| Q2 | Keyboard Navigation | Full keyboard access to all IDE features | Keyboard navigation baseline: command palette, diagnostics, explorer, runtime lanes, Agent Workbench, settings | Keyboard shortcuts exist for commands | Keyboard navigation in palette/diagnostics/explorer/Agent Workbench | `view_ide/commands/` | Keyboard-only navigation test for core actions | No mouse-only interaction paths |
