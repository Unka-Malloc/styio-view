...
16. ShellRuntimeModel 是 Agent 与 Editor 的真实绑定点：它向 Agent context 注入当前 selection、language facts、toolchain snapshot、workspace documentSamples 和 workspace dirty/open 状态；Agent workspace patch 应用后必须更新 editor document、workspace store、open-document cache 与文件清单，不得只改当前 buffer。
