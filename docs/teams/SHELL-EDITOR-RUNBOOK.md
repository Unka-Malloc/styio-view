...
16. Workspace navigation changes must keep `StyioCommandRegistry`, shell bottom-surface routing, workspace services, and runtime file loading in one slice. `Cmd/Ctrl+P` is reserved for shell Quick Open, while editor-local `Ctrl+P` parameter info remains scoped to the editor surface.
