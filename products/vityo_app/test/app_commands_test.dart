...
void main() {
  test('command registry exposes primary command strip in mainline order', () {
    expect(
      VityoCommandRegistry.primaryCommands.map((command) => command.id),
      <AppCommandId>[
        AppCommandId.save,
  ...
  test(
    ...
    () {
      final quickOpen = VityoCommandRegistry.descriptorFor(
        AppCommandId.quickOpen,
      );
      final commandPalette = VityoCommandRegistry.descriptorFor(
        AppCommandId.commandPalette,
      );
      final search = VityoCommandRegistry.descriptorFor(
        AppCommandId.searchWorkspace,
      );
      final recentLocations = VityoCommandRegistry.descriptorFor(
        AppCommandId.showRecentLocations,
      );
      final documentLinks = VityoCommandRegistry.descriptorFor(
        AppCommandId.showWorkspaceDocumentLinks,
      );
      final documentHighlights = VityoCommandRegistry.descriptorFor(
        AppCommandId.showWorkspaceDocumentHighlights,
      );
      final codeLens = VityoCommandRegistry.descriptorFor(
        AppCommandId.showWorkspaceCodeLenses,
      );
      final declaration = VityoCommandRegistry.descriptorFor(
        AppCommandId.goToWorkspaceDeclaration,
      );
      final navigateBack = VityoCommandRegistry.descriptorFor(
        AppCommandId.navigateBack,
      );
      final navigateForward = VityoCommandRegistry.descriptorFor(
        AppCommandId.navigateForward,
      );
      final definition = VityoCommandRegistry.descriptorFor(
        AppCommandId.goToWorkspaceDefinition,
      );
      final typeDefinition = VityoCommandRegistry.descriptorFor(
        AppCommandId.goToWorkspaceTypeDefinition,
      );
      final implementation = VityoCommandRegistry.descriptorFor(
        AppCommandId.goToWorkspaceImplementation,
      );
      final typeHierarchy = VityoCommandRegistry.descriptorFor(
        AppCommandId.showWorkspaceTypeHierarchy,
      );
      final outline = VityoCommandRegistry.descriptorFor(
        AppCommandId.showWorkspaceOutline,
      );
      final rename = VityoCommandRegistry.descriptorFor(
        AppCommandId.renameWorkspaceSymbol,
      );
      final problems = VityoCommandRegistry.descriptorFor(
        AppCommandId.showWorkspaceProblems,
      );
      final codeActions = VityoCommandRegistry.descriptorFor(
        AppCommandId.showWorkspaceCodeActions,
      );
      final symbols = VityoCommandRegistry.descriptorFor(
        AppCommandId.searchWorkspaceSymbols,
      );
      final references = VityoCommandRegistry.descriptorFor(
        AppCommandId.findWorkspaceReferences,
      );
  ...
  test('command registry exposes toolchain and deployment route commands', () {
    expect(
      VityoCommandRegistry.executionCommands.map((command) => command.id),
      <AppCommandId>[AppCommandId.run],
    );
    expect(
      VityoCommandRegistry.searchCommands.map((command) => command.id),
      <AppCommandId>[
        AppCommandId.showRecentLocations,
        AppCommandId.showWorkspaceDocumentLinks,
        AppCommandId.showWorkspaceDocumentHighlights,
    ...
    expect(
      VityoCommandRegistry.navigationCommands.map((command) => command.id),
      <AppCommandId>[
    ...
    expect(
      VityoCommandRegistry.deploymentCommands.map((command) => command.id),
      <AppCommandId>[AppCommandId.packProject, AppCommandId.preparePublish],
    );
    expect(
      VityoCommandRegistry.workflowCommands.map((command) => command.id),
      <AppCommandId>[
        AppCommandId.run,
        AppCommandId.commandPalette,
        AppCommandId.quickOpen,
  ...
  test('command permission service separates allowed, approval, and denied', () {
    ...
    final run = VityoCommandRegistry.descriptorFor(AppCommandId.run);
