...
1. `vityo.themeColor`
2. `vityo.themePalette`
3. `vityo.themeText`
4. `vityo.background`
5. `vityo.themeLines`
6. `vityo.interfaceFont`
7. `vityo.interfaceSize`

这些字段当前接受预设 key / 标签名；默认字体预设命名为 `Default ...`，例如 `defaultSans`，默认金色高亮显示为 `Gold`，其 key 仍为 `defaultGold`。`vityo.interfaceSize` 也支持实际字号数字，例如 `13..17`。
...
1. `vityo.palette`
2. `vityo.editorBackground`
3. `vityo.textColor`
4. `vityo.textHighlight`
5. `vityo.block`
6. `vityo.line`
7. `vityo.selection`
8. `vityo.symbolColors`

`vityo.symbolColors` 接受 glyph key 或 token 作为字段名，例如：
...
如果存在 `vityoView.visualTokens`，这些 token 也会被持久化进同一份 canonical 配置。
