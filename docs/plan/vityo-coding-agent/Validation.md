...
`scripts/vityo_quality.py` runs the Coding Agent package, shared protocol package, integration
...
|---|---|---|
| REQ-AGENT-001 | `python scripts/vityo_quality.py --product coding-agent --suite headless-runtime` | Pure dependency graph plus CLI start/complete/cancel/failure smoke without IDE |
...
```text
python scripts/vityo_quality.py --product coding-agent --suite full
```
...
- real Vityo protocol integration with two concurrent sessions;
