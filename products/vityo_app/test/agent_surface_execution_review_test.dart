...
void main() {
  ...
  testWidgets('agent surface exposes workspace snapshot revert plan', (
    ...
    final patchApplier = AgentCodePatchApplier(
    ...
    await controller.applyPendingPatch(patchApplier);
    _loadRevisionDocument(editorController, revisions, 'main.styio');
    await controller.buildWorkspaceRevertPlan(snapshotService);
    await _pumpSurface(
      tester,
      controller,
      onApplyWorkspaceRevertPlan: () async {
        final result = await controller.applyLastWorkspaceRevertPlan(
          patchApplier,
        );
  ...
  testWidgets('agent surface runs approved workspace patch tools', (
    ...
    await _pumpSurface(
      ...
      onApplyAgentWorkspacePatch: (patch) async {
        appliedPatch = patch;
        final result = await patchApplier.apply(patch);
        if (result.applied) {
          _loadRevisionDocument(editorController, revisions, 'main.styio');
        }
        return result;
