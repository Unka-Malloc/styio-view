...
The atomic cutover creates `scripts/styio_quality.py`, a cross-platform runner that invokes commands
...
|---|---|---|
| REQ-IDE-001 | `python scripts/styio_quality.py --product ide --suite cutover` and `python scripts/check_product_line_boundaries.py` | Both final packages analyze/smoke; old active roots/imports are absent; IDE starts with Agent disabled |
...
- active README/product metadata/packaging use Styio IDE and Coding Agent naming.
...
```text
python scripts/styio_quality.py --product ide --suite full
```
