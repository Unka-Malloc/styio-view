    ...
    return const IdeCapabilityFrameworkSnapshot(
      ...
      entries: <IdeCapabilityDescriptor>[
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'Platform Detector, Platform Context, Platform Adapter, Platform Manager bundle, fact-level health snapshots, PlatformManagerHealthProbe contracts, and UI-facing recovery actions are available for file system, shell, process, resource, network, clipboard, notification, local service, and PTY managers.',
          todo:
              'TODO: replace default lightweight probes with manager-specific live operation probes and bind recovery actions to settings UI.',
          references: <String>[
        ...
        IdeCapabilityDescriptor(
          ...
          ownerPath: 'lib/src/view_ide/language/service',
          summary:
              'SemanticSnapshotProvider converts StyioService analysis into resolved elements/references, exposes StyioService-backed code action raw edit facts, code action apply/result telemetry, document rename safety facts, workspace rename safety facts, SemanticSnapshotEventBridge runtime-output events, Problems/Refactor panel event sink dispatching, publishes a feature coverage matrix for hover/definition/references/completion/rename/code action consumers, and only falls back to local snapshots when service semantic facts are missing.',
          todo:
              'TODO: connect panel event sinks to live Problems/Refactor panel state stores and persisted telemetry.',
          references: <String>[
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'Diagnostics interaction can consume workspace diagnostics provider snapshots, serializable diagnostics filters, persisted panel state, panel-state restoration in ProblemsSurface, source groups, reusable diagnostics view models, quick-fix confirmation plans, WorkspaceQuickFixReviewPlan bridges into concrete Problems diff/apply controls, focused problem actions, and keyboard-navigable Problems panel bindings.',
          todo:
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'Workspace text search service, in-memory search index snapshot, persistent index invalidation key contract, symbol search service, file quick open service, replace preview contract with before/after diff summary, replace apply confirmation, search history persistence, search history/index summaries in the user surface, agent search command, and match-level navigation callback are wired.',
          todo:
              'TODO: add background incremental index refresh execution, persistent result filters, and virtualized multi-file diff expansion.',
          references: <String>[
        ...
        IdeCapabilityDescriptor(
          ...
          ownerPath: 'lib/src/view_ide/workspace',
          summary:
              'Source Control surface is wired to dirty editor documents, Git porcelain status parsing, diff preview, diff review summaries, virtualized diff windows, Agent context snapshots, staging action contracts, persisted commit drafts, commit dialog state validation, commit draft summaries, branch picker summaries, branch switch plans, Git branch switch provider, history summaries, action planning and confirmation, Git stage/unstage/discard/commit action provider, Git branch/history provider contracts, non-Git provider adapter descriptors, file open, and save-all handoff.',
          todo:
              'TODO: bind virtualized diff windows and non-Git adapter descriptors to concrete UI/provider implementations.',
          references: <String>[
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'Testing surface is wired to runTests, native tool testResult records, TestRunProvider registry, TestDiscoveryProvider registry, test run configurations, persisted run configuration sets, run/debug selected configuration controls, test tree model, persisted test run history, runtime task lifecycle snapshots, persisted runtime task history, output stream subscription plans, RuntimeOutputLiveBuffer test-result publishing, failed-test rerun planning, failed-test interaction, rerun-failed handoff, debug launch route plans, failure navigation actions, and CTest result parsing.',
          todo:
              'TODO: connect provider-specific test debug actions to concrete DebugLaunchRoutePlan execution.',
        ...
        IdeCapabilityDescriptor(
          ...
          ownerPath: 'lib/src/view_ide/commands',
          summary:
              'Command Palette surface is wired to StyioCommandRegistry, category contribution manifests, shell command execution, reusable query scoring, overlay selection state, live display preference updates, category filter chips, display preferences, Settings UI preference controls, Up/Down/Enter keyboard navigation, typed input draft contracts, keyboard shortcuts, persisted recent command ranking, recent command record hooks, and blocked command availability reasons.',
          todo:
              'TODO: connect SettingsSurface preference saves to the live preference controller in the top-level shell composition.',
          references: <String>[
        ...
        IdeCapabilityDescriptor(
          ...
          todo:
              'TODO: persist diff pagination state and bind review telemetry into concrete apply-result UI.',
          dependencies: <String>[
            'foundation.registry',
            'workspace.project-model',
          ],
        ...
        IdeCapabilityDescriptor(
          ...
          todo:
              'TODO: replace default runtime manager registrations with concrete shell/toolchain/hosted process execution implementations.',
        ...
        IdeCapabilityDescriptor(
          ...
          ownerPath: 'lib/src/view_ide/agent',
          summary:
              'OpenAI-compatible and OpenAI Responses providers, explicit OpenAI Codex Spark preset, Credential DataStore-backed bearer token references, credential-backed routes, fallback readiness, provider execution context, structured response tool definitions, and extension agent provider contribution manifests.',
        ),
        IdeCapabilityDescriptor(
          ...
          ownerPath: 'lib/src/view_ide/agent',
          summary:
              'Structured plan, diagnostics, code patch, IDE command, command result, WorkspaceEdit bridge, Source Control Agent context bridge, failed-test rerun context, StyioService readiness checkpoints, checkpoint result prompt replay, patch application loop, persisted coding session history, and embeddable activity history surface.',
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'Active document diagnostics panel is wired into the IDE shell and has workspace diagnostics grouping, source grouping, severity filter, provider contract, per-diagnostic quick-fix selection, quick-fix confirmation planning, WorkspaceQuickFixReviewPlan preview/control contracts, concrete diff/apply controls, preview, and navigation available.',
          todo:
              'TODO: add virtualized multi-file diff expansion and shell command routing for per-fix apply results.',
          dependencies: <String>['workspace.diagnostics'],
          references: <String>[
