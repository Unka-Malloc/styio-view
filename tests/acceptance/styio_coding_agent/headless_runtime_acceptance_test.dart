...
Future<void> _reportsStructuredCliOutcomes() async {
  ...
  final product = Directory.fromUri(
    repository.uri.resolve('products/styio_coding_agent/'),
  );
  final cli = 'bin/styio_coding_agent.dart';
...
void _rejectsForbiddenRuntimeDependencies() {
  ...
  final pubspec = File.fromUri(product.uri.resolve('pubspec.yaml'))
      .readAsStringSync();
  _expect(
    !pubspec.contains('flutter:') && !pubspec.contains('styio_ide:'),
    'the Coding Agent pubspec must not depend on Flutter or Styio IDE',
  );
    ...
    for (final file in root
      ...
      _expect(
        !source.contains('AgentClientConnection') &&
            !source.contains('StyioCodingAgentRuntime'),
