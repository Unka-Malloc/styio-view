...
### 0.2 Documentation Language

1. Repository documentation is English by default.
2. Chinese prose is allowed only in documents explicitly scoped as Chinese localization, Chinese translation, or Chinese user-facing product/marketing copy, such as a localized `README.md` or product landing page.
3. Specs, ADRs, design docs, runbooks, plans, reviews, rollups, governance docs, generated indexes, and implementation-decision summaries must not receive new Chinese prose unless their `Purpose` explicitly declares a Chinese-localized scope.
4. Existing Chinese prose in non-localized owner docs is legacy content until touched. When materially editing such text, convert the edited passage to English instead of extending the Chinese section.
...
3. 新增一个长期架构边界时，必须新增或更新设计文档；仍需直接评审时新增 ADR，实现并吸收到 owner SSOT 后压缩进 `docs/adr/IMPLEMENTED-DECISIONS.md`，不能只写在缺口登记里。
...
9. 根 `.gitignore` 若扩展 temp/build/log/cache 忽略规则，必须同批补 `docs/**` 与 `products/vityo_app/test/**` 的显式 negate 规则，并让 `python3 scripts/repo-hygiene-gate.py --mode tracked` 通过。
...
4. 任何实现如果违反已接受 ADR 或 `docs/adr/IMPLEMENTED-DECISIONS.md` 中仍为当前事实的决策摘要，必须先新增替代 ADR、更新 owner SSOT，或回滚计划。
