    ...
    return const IdeCapabilityFrameworkSnapshot(
      ...
      entries: <IdeCapabilityDescriptor>[
        ...
        IdeCapabilityDescriptor(
          ...
          ownerPath: 'lib/src/view_ide/language/service',
          summary:
              'SemanticSnapshotProvider converts StyioService analysis into resolved elements/references, exposes StyioService-backed code action raw edit facts, code action apply/result telemetry, document rename safety facts, workspace rename safety facts, SemanticSnapshotEventBridge runtime-output events, Problems/Refactor panel event sink dispatching, SemanticSnapshotPanelEventStateController, SemanticSnapshotPanelViewModel Problems/Refactor projections, concrete Problems panel projection rendering, SemanticSnapshotPanelEventStore persisted telemetry, publishes a feature coverage matrix for hover/definition/references/completion/rename/code action consumers, and only falls back to local snapshots when service semantic facts are missing.',
          todo:
              'TODO: bind SemanticSnapshotPanelViewModel to concrete Refactor widgets and production telemetry retention.',
          references: <String>[
        ...
        IdeCapabilityDescriptor(
          ...
          ownerPath: 'lib/src/view_ide/commands',
          summary:
              'Command Palette surface is wired to StyioCommandRegistry, category contribution manifests, shell command execution, reusable query scoring, overlay selection state, live display preference updates, top-level SettingsSurface preference saves, shell-level DataStore preference hydration/persistence, app bootstrap preference hydration, category filter chips, display preferences, Settings UI preference controls, Up/Down/Enter keyboard navigation, typed input draft contracts, keyboard shortcuts, keybinding remap persistence, surface-level keybinding editor, conflict review contracts and actions, persisted recent command ranking, recent command record hooks, and blocked command availability reasons.',
          todo:
        ...
        IdeCapabilityDescriptor(
          ...
          summary:
              'Active document diagnostics panel is wired into the IDE shell and has workspace diagnostics grouping, source grouping, severity filter, provider contract, per-diagnostic quick-fix selection, per-fix preview/apply command routing, quick-fix confirmation planning, WorkspaceQuickFixReviewPlan preview/control contracts, concrete diff/apply controls, preview, and navigation available.',
          todo:
              'TODO: add virtualized multi-file diff expansion and persisted per-fix apply result rows.',
          dependencies: <String>['workspace.diagnostics'],
          references: <String>[
