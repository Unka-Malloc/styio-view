...
| Local bridge / cloud execution for AI | Partially implemented | Vityo | `AgentProviderRouteExecutor` now normalizes cloud, loopback local-bridge, and blocked execution plans from provider route + endpoint + platform local-service compatibility. `ConfiguredAgentProviderAdapterFactory` consumes the plan, marks loopback providers as `localBridge`, can route local bridge requests through a dedicated transport while AppBootstrap supplies `platformManagers.localService`, and can fail over from a blocked primary endpoint to serialized profile fallback endpoints. Remaining closure: product setup UI, hosted/local route health checks, and user-visible failover recovery controls. |
